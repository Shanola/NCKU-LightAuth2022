...
#define CIPHER_LIST "TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384"
...
int main()
{
    ...
    while (!shutdown) {
        ...
        if ((ret = wolfSSL_read(ssl, buff, sizeof(buff))) == -1) {
            fprintf(stderr, "ERROR: failed to read\n");
            goto exit;
        }

        /* Print to stdout any data the client sends */
        printf("Length of Client's msg: %d\n", ret);
        ...
        if (wolfSSL_write(ssl, buff, ret) != ret) {
