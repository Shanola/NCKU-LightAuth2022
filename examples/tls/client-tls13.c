...
#define CIPHER_LIST "TLS13-AES256-GCM-SHA384"
...
int main(int argc, char** argv)
{
    ...
    if (wolfSSL_write(ssl, buff, sizeof(buff)) < ret) {
    ...
    memset(buff, 0, sizeof(buff));
    if ((ret = wolfSSL_read(ssl, buff, sizeof(buff))) < 0) {
        fprintf(stderr, "ERROR: failed to read\n");
        goto exit;
    }
	double t2 = tvgetf();
    /* Print to stdout any data the server sends */
    printf("Length of Server's message: %d\n", ret);
	printf("%f\n", (t2-t1)*1000);

	FILE *fp = fopen("auth_enc_tls13.txt", "a");
	char ctmp[10];
